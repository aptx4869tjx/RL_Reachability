...
from abstract.b3.b3_abs import *
...
def task(region, cnt, info=False):
    ...
    info_list = [r]
    while True:
        bound_list = b3_2.get_next_bound_list(bound_list)
        if info:
            min_x1 = 100
            max_x1 = -100
            min_x2 = 100
        ...
        if t == 150:
            if len(bound_list) == 0:
                print('verified', cnt)
                if info:
                    np.save('./his/b3_box_history' + str(cnt), arr=np.array(info_list))
                return True, b3_2.time_seg, b3_2.time_op, b3_2.time_agg
            else:
                print('exception', region, cnt, len(bound_list))
                if info:
                    np.save('./his/b3_box_history' + str(cnt), arr=np.array(info_list))
                return False, b3_2.time_seg, b3_2.time_op, b3_2.time_agg
...
if __name__ == "__main__":
    ...
    agent.load()
    # evaluate(agent)
    b3_2 = B3_Env(divide_tool, agent.network)
