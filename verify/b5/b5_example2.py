...
from abstract.b5.b5_abs import state_space, Agent, train_model, initial_intervals, evaluate, evaluate_trace
...
# np.save('./traces/b5_trace_list', arr=trace_list)
b5 = B5_Env2(divide_tool, agent.actor)

# high = np.array([0.4, 0.47, 0.27])
# low = np.array([0.38, 0.45, 0.25])

r = [0.38, 0.45, 0.25, 0.4, 0.47, 0.27]
# r = [0.38, 0.45, 0.25, 0.39, 0.46, 0.252]
# r = [0.38, 0.45, 0.263, 0.39, 0.46, 0.264]
# r = [0.395, 0.465, 0.25, 0.4, 0.47, 0.251]
# r = [0.39, 0.45, 0.25, 0.4, 0.46, 0.251]
t = 0
st = time.time()
bound_list = [r]
interval_num_agg = []
while True:
    t0 = time.time()
    bound_list = b5.get_next_bound_list(bound_list)
    min_x1 = 100
    max_x1 = -100
    min_x2 = 100
    max_x2 = -100
    ...
    if t == 50:
        break
et = time.time()
np.save('b5_interval_number_no_agg', arr=interval_num_agg)
print(et - st)
