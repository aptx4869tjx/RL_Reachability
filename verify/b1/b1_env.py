...
from verify.divide_tool import str_to_list, combine_bound_list, max_min_clip, contain


class B1_Env():

    def __init__(self, divide_tool, network, scala=4, offset=0, std=[0.0001, 0.0001]):
        self.action = 0.0
    ...
    def get_next_states(self, current):
        ...
        if current[0] >= 0 and current[2] <= 0.2 and current[1] >= 0.05 and current[3] <= 0.3:
            return [], 0
    ...
    def gn(self, current):
        ...
        p_l = pl + vl * t
        p_r = pr + vr * t
        v_l = vl + (self.action * vl * vl - pr) * t
        v_r = vr + (self.action * vr * vr - pl) * t
        p_l = np.clip(p_l, -2, 2)
        p_r = np.clip(p_r, -2, 2)
        v_l = np.clip(v_l, -2, 2)
        v_r = np.clip(v_r, -2, 2)
...
def near_bound(current, target):
    ...
    # standard = [0.00001, 0.00001]
    standard = [0.0001, 0.0001]  # tanh 2_20 tanh 3_100
    # standard = [0.001, 0.001] # relu 2_20 relu 3_100
    # standard = [0.0005, 0.0005]
    dim = len(current)
    half_dim = int(dim / 2)
    counter = 0
    record_dim = None
