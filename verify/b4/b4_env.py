...
class B4_Env():
    ...
    def get_next_states(self, current):
        ...
        try:
            ...
            print(current)
            exit(0)
        if current[0] > -0.05 and current[3] < 0.05 and current[1] > -0.05 and current[4] < 0:
            return [], 0
    ...
    def gn(self, current):
        ...
        x2r = current[4]
        x3r = current[5]
        bound = Bounds([x1l, x2l, x3l], [x1r, x2r, x3r])

        # x1_new = x1 + (-x1 + x2 - x3) * t
        # x2_new = x2 + (-x1 * (x3 + 1) - x2) * t
        # x3_new = x3 + (-x1 + u) * t

        x1_l = x1l + (-x1l + x2l - x3r) * t
        x1_r = x1r + (-x1r + x2r - x3l) * t

        x0 = [(x1l + x1r) / 2, (x2l + x2r) / 2, (x3l + x3r) / 2]
        ...
        x3_l = x3l + (-x1r + self.action) * t
        x3_r = x3r + (-x1l + self.action) * t

        x1_l = np.clip(x1_l, -2, 2)
        x1_r = np.clip(x1_r, -2, 2)
        x2_l = np.clip(x2_l, -2, 2)
        x2_r = np.clip(x2_r, -2, 2)
        x3_l = np.clip(x3_l, -2, 2)
        x3_r = np.clip(x3_r, -2, 2)
    ...
    def x2_minimum(self, x):
        return x[1] + (-x[0] * (x[2] + 1) - x[1]) * self.tau
